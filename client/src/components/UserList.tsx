...
import WebSocketClient, { User, WebSocketStatus } from '../services/websocket.service';

const UserList: React.FC = () => {
  const [users, setUsers] = useState<User[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [connectionStatus, setConnectionStatus] = useState<WebSocketStatus>(WebSocketStatus.CLOSED);
      ...
      <div className="controls">
        <div className="status">
          ...
          <span className={`status-${connectionStatus}`}>
            {connectionStatus === WebSocketStatus.OPEN ? '연결됨' :
             connectionStatus === WebSocketStatus.CONNECTING ? '연결 중...' :
             connectionStatus === WebSocketStatus.CLOSING ? '연결 종료 중...' : '연결 끊김'}
          </span>
        </div>
        
        <div className="actions">
          <button onClick={handleRefreshUsers} disabled={connectionStatus !== WebSocketStatus.OPEN}>
            사용자 목록 새로고침
          </button>
        </div>
